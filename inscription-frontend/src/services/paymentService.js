...
// Import the pre-configured axios instance instead of using fetch
import api from './api';

// The endpoint path relative to the base URL in api.js
...
export const createCheckoutSession = async (paymentDetails) => {
    try {
        ...
        const response = await api.post(`${PAYMENT_URL}/create-checkout-session`, paymentDetails);
